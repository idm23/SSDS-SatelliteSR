...
def main(args):
    ...
    trainset = ImgDatasets(root_dir='data/img_celeba_big', files='train_files.txt', mode=args.mode)
    trainloader = data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)

    testset = ImgDatasets(root_dir='data/img_celeba_big', files='test_files.txt', mode=args.mode)
    ...
    net = Glow(num_channels=args.num_channels,
               ...
               mode=args.mode,
               cond_channels = args.num_cond_channels)
    net = net.to(device)
    if device == 'cuda':
    ...
    if args.resume:
        ...
        if args.resume_best:
            ...
            print('Resuming from checkpoint at ckpts/recent.pth.tar...')
            assert os.path.isdir('ckpts'), 'Error: no checkpoint directory found!'
            checkpoint = torch.load('ckpts/recent.pth.tar')
...
def test(epoch, net, testloader, device, loss_fn, mode='color'):
    ...
    if loss_meter.avg < best_loss:
        ...
        os.makedirs('ckpts', exist_ok=True)
        torch.save(state, 'ckpts/recent.pth.tar')
    origin_img, cond_img = next(iter(testloader))
    B = cond_img.shape[0]
    # Save samples and data
    images = sample(net, cond_img, device)
    os.makedirs('samples', exist_ok=True)
    os.makedirs('ref_pics', exist_ok=True)
    if mode == 'sketch':
        cond_img = (~cond_img.type(torch.bool)).type(torch.float)
    ...
    cond_concat = torchvision.utils.make_grid(cond_img, nrow=int(B ** 0.5), padding=2, pad_value=255)

    torchvision.utils.save_image(images_concat, 'samples/epoch_{}.png'.format(epoch))
    torchvision.utils.save_image(origin_concat, 'ref_pics/origin_{}.png'.format(epoch))
    torchvision.utils.save_image(cond_concat, 'ref_pics/cond_{}.png'.format(epoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_cond_channels', default=4, type=int, help='Number of cond channels')
    ...
    parser.add_argument('--resume_best', type=str2bool, default=False, help='Resume from best checkpoint')
    parser.add_argument('--seed', type=int, default=0, help='Random seed for reproducibility')
    parser.add_argument('--warm_up', default=500000, type=int, help='Number of steps for lr warm-up')
    parser.add_argument('--mode', default="sketch", choices=['gray', 'sketch', 'decimated'])
    best_loss = float('inf')
    global_step = 0
