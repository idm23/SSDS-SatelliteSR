...
class ImgDatasets(torch.utils.data.Dataset):
    def __init__(self, root_dir, files, mode='sketch'):
        ...
        self.decimate_transform = transforms.Compose([
    ...
    def __getitem__(self, index):
        ...
        dec_image = self.decimate_transform(image)
        image = self.ToTensor(image)
        gray_img = self.ToTensor(gray_img)
        dec_image = self.ToTensor(dec_image)
        if self.mode == 'gray':
            return (image, gray_img)
        if self.mode == 'decimated':
            return (image, dec_image)
        else:
